...
<br>埋め込みプレイヤーのJavaScript APIが無効のままだったため、再生位置の保存も続きから再生も実際には動作していませんでした。公式ドキュメントに従い埋め込みURLに api=1 を付与してAPIを有効化し、再生位置を保存できるようにしました。あわせて、再生開始前は効かない seek に頼らず埋め込みURLの start パラメータで開始位置を指定して確実に頭出しし、公式アプリと同じ基準（30秒未満の短い動画や、残り10秒以内まで視聴済みの動画は先頭から再生）で続きから再生するか判定します
